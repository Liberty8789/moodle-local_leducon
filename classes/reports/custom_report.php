...
class custom_report extends base_report {
        ...
        switch ($ds) {
            ...
            case 'users':
                $cols = [
                    ...
                    'city'          => ['label' => 'City',         'sql' => 'u.city',            'type' => 'text'],
                    'country'       => ['label' => 'Country',      'sql' => 'u.country',         'type' => 'text'],
                    'cohortname'    => ['label' => 'Cohort',       'sql' => '(SELECT MIN(coh.name) FROM {cohort_members} cm_u JOIN {cohort} coh ON coh.id = cm_u.cohortid WHERE cm_u.userid = u.id)', 'type' => 'text'],
                    'orgunitname'   => ['label' => 'Org Unit',     'sql' => '(SELECT MIN(ou.name) FROM {local_leducon_org_members} om_u JOIN {local_leducon_org_units} ou ON ou.id = om_u.orgunitid WHERE om_u.userid = u.id)', 'type' => 'text'],
    ...
    public static function get_templates(): array {
        return [
            ...
            'user_directory' => [
                'name'        => get_string('tpl_user_directory', 'local_leducon'),
                'description' => get_string('tpl_user_directory_desc', 'local_leducon'),
                'datasource'  => 'users',
                'config'      => [
                    'columns'  => ['fullname', 'email', 'username', 'department', 'institution', 'city', 'country', 'cohortname', 'orgunitname', 'lastaccess', 'timecreated', 'suspended'],
                    'conditions' => [],
                    'scope'    => [],
                    'orderby'  => 'fullname',
                    'orderdir' => 'ASC',
